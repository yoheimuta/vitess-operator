...
function get_started_unmanaged() {
    ...
    # echo "Apply latest operator.yaml"
    # kubectl apply -f "operator.yaml"
    checkPodStatusWithTimeout "vitess-operator(.*)1/1(.*)Running(.*)"

    echo "Apply 101_initial_cluster_unmanaged.yaml"
    kubectl apply -f "101_initial_cluster_unmanaged.yaml"
    ...
    checkPodStatusWithTimeout "example-vttablet-zone1(.*)3/3(.*)Running(.*)"
...
# In get_started_unmanaged we verify that the pod for vtadmin exists and is healthy
